<?php
 ...
 if (isset($_GET["play_file_name_one"]))       {$play_file_name_one=$_GET["play_file_name_one"];}
  ...
  if (isset($_GET["telemarketingdltid"]))       {$telemarketingdltid=$_GET["telemarketingdltid"];}
  elseif (isset($_POST["telemarketingdltid"]))  {$telemarketingdltid=$_POST["telemarketingdltid"];} 
  ...
  if (isset($_GET["SMSSenderIDAction_id"]))       {$SMSSenderIDAction_id=$_GET["SMSSenderIDAction_id"];}
  elseif (isset($_POST["SMSSenderIDAction_id"]))  {$SMSSenderIDAction_id=$_POST["SMSSenderIDAction_id"];}
...
if($action=='add_sms_senderid'){

	
			$sql = "SELECT * FROM `sms_senderid` where senderid='$senderid'";
			...
			if($rowscount==0){
				...
				$sql = "INSERT INTO sms_senderid (`senderid`, `status`,`type`,`dltsenderid`,`telemarketingdltid`,`entityid`,`remarks`,`request_datetime`,`username`) VALUES ('$senderid','process','$type','$dltsenderid','$telemarketingdltid','$entityid','','$now','$login_id')";
					...
					if(mysqli_query($conn, $sql))
					{
						...
						$response   = "200";
						$output = array("Response"=>$response,"message"=>"SenderID request add successfully");
					}
					else{
						$data       = "Error !";
						$response   = "400";
...
if($action=='SMSSenderIDAction_form'){
  
      $sql = "SELECT * FROM `sms_senderid` where id='$SMSSenderIDAction_id'";
      $res = mysqli_query($conn, $sql);
      $rowscount = mysqli_num_rows($res);
      if($rowscount>0){

        if($status=='Pending'){
          $sql = "UPDATE sms_senderid SET `status`='$status' WHERE id='$SMSSenderIDAction_id'";
        }
        elseif ($status=='Approve') {
          $sql = "UPDATE sms_senderid SET `status`='$status',`remarks`='$approvalremarks',`approve_datetime`='$now' WHERE id='$SMSSenderIDAction_id'";
        }
        elseif ($status=='Reject') {
          $sql = "UPDATE sms_senderid SET `status`='$status',`remarks`='$approvalremarks',`reject_datetime`='$now' WHERE id='$SMSSenderIDAction_id'";
        }
          ...
          if(mysqli_query($conn, $sql))
          {
            ...
            $response   = "200";
            $output = array("Response"=>$response,"message"=>"SMS sender ID status updated successfully");
          }
          else{
            $data       = "Error !";
            $response   = "400";

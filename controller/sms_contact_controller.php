...
if($action=='add_sms_contact'){

	if(!empty($contact_id)){

			$sql = "SELECT * FROM `sms_contact` where mobile='$mobile' and contact_id!='$contact_id'";
			$res = mysqli_query($conn, $sql);
		 	$rowscount = mysqli_num_rows($res);
			if($rowscount==0){

				$sql = "UPDATE sms_contact SET `group_id`='$group_id',`firstname`='$firstname',`lastname`='$lastname',`mobile`='$mobile',`email`='$email',`overwritecontact`='$overwritecontact',`desc`='$desc'  WHERE contact_id='$contact_id' ";
			...
			$sql = "SELECT * FROM `sms_contact` where mobile='$mobile'";
			$res = mysqli_query($conn, $sql);
		 	$rowscount = mysqli_num_rows($res);
			if($rowscount==0){
				...
				$sql = "INSERT INTO sms_contact (`group_id`,`firstname`,`lastname`,`mobile`,`email`,`overwritecontact`,`desc`,`created_date`,`username`) VALUES ('$group_id','$firstname','$lastname','$mobile','$email','$overwritecontact','$desc','$now','$login_id')";
...
if($action=='sms_contact_delete'){

	$sql = "DELETE FROM `sms_contact` WHERE contact_id='$contact_id'";
	$res = mysqli_query($conn, $sql);
